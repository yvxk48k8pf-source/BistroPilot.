      ...
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        {slices.map((s, i) => (
          <circle key={i} cx={cx} cy={cy} r={r}
            ...
            strokeDasharray={`${s.pctVal * circ} ${circ}`}
            strokeDashoffset={-s.offset * circ}
            style={{ transition: "stroke-dasharray 0.8s ease" }} />
        ))}
      </svg>
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: 20, fontWeight: 900, color: "#1A1A2E" }}>{euro(total)}</div>
...
function AIPanel({ state, onClose }) {
  const [msgs, setMsgs] = useState([{ role: "assistant", text: `Bonjour ! Je suis votre analyste IA pour **${state.nom}**. Posez-moi n'importe quelle question financière.` }]);
     ...
     const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              ...msgs.filter((m, i) => i > 0).concat({ role: "user", content: msg }).map(m => ({
                role: m.role,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "flex-end", justifyContent: "flex-end" }}>
      <div style={{ background: "#fff", borderRadius: "20px 20px 0 0", width: 420, height: "75vh", display: "flex", flexDirection: "column", boxShadow: "0 -8px 40px rgba(0,0,0,0.15)" }}>
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #F3F4F6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#9CA3AF" }}>Analyse financière en temps réel</div>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px", display: "flex", flexDirection: "column", gap: 12 }}>
          {msgs.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
              <div style={{ maxWidth: "85%", background: m.role === "user" ? "#F97316" : "#F9FAFB", color: m.role === "user" ? "#fff" : "#1A1A2E", borderRadius: m.role === "user" ? "18px 18px 4px 18px" : "18px 18px 18px 4px", padding: "10px 14px", fontSize: 13, lineHeight: 1.6, border: m.role === "assistant" ? "1px solid #E5E7EB" : "none", fontWeight: m.role === "user" ? 600 : 400 }}>{m.text}</div>
            </div>
          ))}
          {loading && <div style={{ display: "flex", gap: 5, padding: "10px 14px", background: "#F9FAFB", border: "1px solid #E5E7EB", borderRadius: "18px 18px 18px 4px", width: "fit-content" }}>
        ...
        <div style={{ padding: "12px 24px 24px", borderTop: "1px solid #F3F4F6", display: "flex", gap: 8 }}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()}
            ...
            style={{ flex: 1, border: "1px solid #E5E7EB", borderRadius: 12, padding: "10px 14px", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
          <button onClick={send} disabled={loading} style={{ background: "#F97316", color: "#fff", border: "none", borderRadius: 12, padding: "10px 16px", cursor: "pointer", fontSize: 16, opacity: loading ? 0.5 : 1 }}>➤</button>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "28px 32px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#fff", border: "1px solid #E5E7EB", borderRadius: 10, padding: "8px 16px", fontSize: 13, color: "#374151", cursor: "pointer" }}>
              📅 Avril 2026
            </div>
        ...
        {page === "dashboard" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
              ...
              <KPICard label="Dépenses totales" value={euro(totalDep)} evol={depEvol} icon="💼" color="#EF4444" />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 16 }}>
              ...
              <Card>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  ...
                  <div style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", borderRadius: 8, padding: "5px 12px", fontSize: 12, color: "#374151" }}>Cette semaine</div>
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                  {["0€", "5k€", "10k€", "15k€", "20k€"].map((v, i) => (
                    <span key={i} style={{ fontSize: 10, color: "#D1D5DB" }}>{v}</span>
                  ))}
                </div>
              </Card>
              ...
              <Card>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#F97316", cursor: "pointer" }}>Voir le détail →</span>
                </div>
                  ...
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
                    {donutData.map((d, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 700, color: "#1A1A2E" }}>{euro(d.value)} ({pct(d.value, totalCharges)}%)</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 16 }}>
              ...
              <Card>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  ...
                  <span style={{ fontSize: 12, color: "#F97316", cursor: "pointer" }} onClick={() => setPage("depenses")}>Voir toutes les dépenses →</span>
                </div>
                  ...
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 0", borderBottom: i < 3 ? "1px solid #F9FAFB" : "none" }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: cat.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>
                        {d.categorie === "alimentaire" ? "🥩" : d.categorie === "boissons" ? "🍷" : d.categorie === "charges" ? "🏠" : "📦"}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 14, color: "#1A1A2E" }}>{d.label}</div>
                        <div style={{ fontSize: 12, color: "#9CA3AF" }}>Fournisseur : {d.fournisseur || "—"}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontWeight: 800, fontSize: 14, color: "#1A1A2E" }}>{euro(d.montant)}</div>
              ...
              <Card>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div style={{ fontWeight: 800, fontSize: 15, color: "#1A1A2E" }}>Budget vs Réalisé</div>
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ height: 10, background: "#F3F4F6", borderRadius: 10, overflow: "hidden" }}>
                    <div style={{ height: "100%", borderRadius: 10, background: "linear-gradient(90deg, #22C55E, #86EFAC)", width: `${pct(totalCharges, state.budgetMensuel)}%`, transition: "width 0.8s ease" }} />
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                    <span style={{ fontSize: 12, color: "#6B7280" }}>Réalisé : {euro(totalCharges)}</span>
                    <span style={{ fontSize: 12, color: "#6B7280", fontWeight: 600 }}>{pct(totalCharges, state.budgetMensuel)}% du budget</span>
                  </div>
                </div>
                  ...
                  {donutData.slice(0, 3).map((d, i) => (
                    <div key={i}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontSize: 12, color: "#374151" }}>{d.label}</span>
                        <span style={{ fontSize: 12, fontWeight: 700 }}>{euro(d.value)}</span>
                      </div>
                      <div style={{ height: 5, background: "#F3F4F6", borderRadius: 5, overflow: "hidden" }}>
                        <div style={{ height: "100%", borderRadius: 5, background: d.color, width: `${pct(d.value, state.budgetMensuel)}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
              {Object.entries(CATS).slice(0, 4).map(([k, v]) => {
                const total = k === "salaires" ? state.employes.reduce((s, e) => s + e.salaire, 0) : state.depenses.filter(d => d.categorie === k).reduce((s, d) => s + d.montant, 0);
                return total > 0 ? (
                ...
                <thead>
                  <tr style={{ borderBottom: "2px solid #F3F4F6" }}>
                    {["Dépense", "Fournisseur", "Catégorie", "Date", "Montant", ""].map(h => (
                      <th key={h} style={{ textAlign: "left", padding: "10px 12px", fontSize: 12, fontWeight: 700, color: "#9CA3AF", letterSpacing: 0.5 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {state.depenses.map((d, i) => {
                    const cat = CATS[d.categorie] || CATS.autres;
                    return (
                      <tr key={d.id} style={{ borderBottom: "1px solid #F9FAFB", transition: "background 0.1s" }}>
                        <td style={{ padding: "12px", fontWeight: 700, fontSize: 14, color: "#1A1A2E" }}>{d.label}</td>
                        ...
                        <td style={{ padding: "12px" }}>
                          <button onClick={() => setState(s => ({ ...s, depenses: s.depenses.filter(x => x.id !== d.id) }))}
                            style={{ background: "#FEF2F2", color: "#EF4444", border: "none", borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontSize: 13 }}>✕</button>
                        </td>
                      </tr>
                    );
                  })}
            ...
            {state.employes.map(emp => (
              <Card key={emp.id}>
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: "linear-gradient(135deg, #F97316, #FBBF24)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 900, color: "#fff" }}>
                    {emp.nom.split(" ").map(w => w[0]).join("")}
                  </div>
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 15, color: "#1A1A2E" }}>{emp.nom}</div>
                ...
                <div style={{ borderTop: "1px solid #F3F4F6", paddingTop: 12, display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span style={{ fontSize: 20, fontWeight: 900, color: "#1A1A2E" }}>{euro(emp.salaire)}</span>
                </div>
              </Card>
            ))}
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {state.plats.sort((a, b) => ((b.prix - b.cout) / b.prix) - ((a.prix - a.cout) / a.prix)).map((p, i) => {
                  ...
                  const benef = (p.prix - p.cout) * p.ventes;
                  const color = marge > 60 ? "#22C55E" : marge > 40 ? "#F59E0B" : "#EF4444";
                  return (
                    <tr key={i} style={{ borderBottom: "1px solid #F9FAFB" }}>
                      ...
                      <td style={{ padding: "14px 12px" }}>
                        <span style={{ background: color + "15", color, borderRadius: 20, padding: "3px 10px", fontSize: 12, fontWeight: 700 }}>{Math.round(marge)}%</span>
                      </td>
                      <td style={{ padding: "14px 12px", fontSize: 13 }}>{p.ventes}</td>
                      <td style={{ padding: "14px 12px", fontWeight: 800, fontSize: 15, color: "#22C55E" }}>{euro(benef)}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {page === "budget" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <Card>
                <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 20 }}>Objectif mensuel</div>
                <div style={{ fontSize: 42, fontWeight: 900, color: "#1A1A2E", letterSpacing: "-1px", marginBottom: 4 }}>{euro(state.budgetMensuel)}</div>
                <div style={{ fontSize: 13, color: "#9CA3AF", marginBottom: 16 }}>Budget défini pour Avril 2026</div>
                <div style={{ height: 12, background: "#F3F4F6", borderRadius: 12, overflow: "hidden", marginBottom: 8 }}>
            ...
            {[
              ...
              <Card key={i} style={{ cursor: "pointer", transition: "box-shadow 0.2s" }}>
                <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 15, color: "#1A1A2E" }}>{r.title}</div>
      ...
      {showAddDep && (
        <div onClick={() => setShowAddDep(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, padding: 32, width: 480, boxShadow: "0 20px 60px rgba(0,0,0,0.15)" }}>
            <div style={{ fontWeight: 900, fontSize: 20, marginBottom: 24, color: "#1A1A2E" }}>Nouvelle dépense</div>
